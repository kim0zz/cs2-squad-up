...
import { addEventComment } from "@/lib/eventRepository";
import { NICKNAME_KEY } from "@/lib/eventRules";
...
export function EventDiscussion({
  ...
  const [nick, setNick] = useState(() => localStorage.getItem(NICKNAME_KEY) ?? "");
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true);
    try {
      await addEventComment(eventId, trimmedNick, trimmedBody);
      ...
      {!formOpen ? (
        ...
        <div className="space-y-3 border-t border-border/50 pt-3">
          <div className="space-y-1.5">
            <Label htmlFor="event-discussion-nick">Nick</Label>
            <Input
              id="event-discussion-nick"
              value={nick}
          ...
          <div className="space-y-1.5">
            <Label htmlFor="event-discussion-body">Wiadomość</Label>
            <Textarea
              id="event-discussion-body"
              value={body}
              onChange={(e) => setBody(e.target.value)}
              maxLength={BODY_MAX}
              placeholder="Np. Gram AWP…"
