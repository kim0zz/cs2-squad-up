...
import { addFootballOccurrenceComment } from "@/lib/footballRepository";
import { FOOTBALL_NICKNAME_KEY } from "@/lib/footballRules";
import type { FootballOccurrenceComment } from "@/types/football";
...
interface FootballDiscussionProps {
  occurrenceId: string;
  comments: FootballOccurrenceComment[];
  onCommentsRefresh: () => Promise<void>;
}

export function FootballDiscussion({
  occurrenceId,
  comments,
  onCommentsRefresh,
}: FootballDiscussionProps) {
  const [formOpen, setFormOpen] = useState(false);
  const [nick, setNick] = useState(() => localStorage.getItem(FOOTBALL_NICKNAME_KEY) ?? "");
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true);
    try {
      await addFootballOccurrenceComment(occurrenceId, trimmedNick, trimmedBody);
  ...
  return (
    <Card className="space-y-3 border-border/80 bg-gradient-card p-4 sm:p-5">
      <h2 className="font-display text-sm font-bold uppercase tracking-wide text-sky-400/90 border-b border-border/50 pb-2">
        Dyskusja
      </h2>
      ...
      {!formOpen ? (
        ...
        <div className="space-y-3 border-t border-border/50 pt-3">
          <div className="space-y-1.5">
            <Label htmlFor="football-discussion-nick">Nick</Label>
            <Input
              id="football-discussion-nick"
              value={nick}
          ...
          <div className="space-y-1.5">
            <Label htmlFor="football-discussion-body">Wiadomość</Label>
            <Textarea
              id="football-discussion-body"
              value={body}
              onChange={(e) => setBody(e.target.value)}
              maxLength={BODY_MAX}
              placeholder="Np. Mam piłkę…"
